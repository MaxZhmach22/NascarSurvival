...
Shader "OCTAVIAN/SliderRampSpecularOutline"
{
	Properties
	{
	...
	SubShader
	{
		...
		v2f TCP2_Outline_Vert(a2v v)
		{
            ...
            float dist = length(world.xyz - _PlayerPosition.xyz);
            //float3 p = world.xyz - _BendOrigin.xyz;
            //float distsqr = dot(p, p);
            dist = max(0, dist - FALLOFF_STRENGTH);
            dist = pow(dist, 1);
            world.xyz += dist * BEND_AMOUNT;
            //float4 value = mul(unity_WorldToObject, world);
            //o.pos = UnityObjectToClipPos(value);
		...
		#pragma surface surf ToonyColorsCustom  exclude_path:deferred exclude_path:prepass vertex:vert
		#pragma target 3.0
		#include "MyCurvey.cginc"
